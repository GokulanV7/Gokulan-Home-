...
function Missile({ id, startPosition, targetPosition, speed = 0.8, onHit, onMiss }: MissileProps) {
  const missileRef = useRef<THREE.Mesh>(null); const totalDistance = useRef(startPosition.distanceTo(targetPosition)); const distanceTraveled = useRef(0);
  useEffect(() => { if (missileRef.current) { missileRef.current.position.copy(startPosition); missileRef.current.lookAt(targetPosition); } }, [startPosition, targetPosition]);
  useFrame((_, delta) => {
    if (!missileRef.current) return; const travelDelta = speed * delta * 30; distanceTraveled.current += travelDelta;
    if (distanceTraveled.current >= totalDistance.current) { missileRef.current.position.copy(targetPosition); onHit(id, targetPosition.clone()); return; }
    const direction = new THREE.Vector3().subVectors(targetPosition, startPosition).normalize(); missileRef.current.position.addScaledVector(direction, travelDelta);
    if (distanceTraveled.current > totalDistance.current + 10 && onMiss) onMiss(id); 
  });
  return ( <mesh ref={missileRef}> <Trail width={0.3} length={5} color={new THREE.Color("#FFD700")} attenuation={(w) => w * 0.8}> <mesh rotation-x={Math.PI / 2}> <cylinderGeometry args={[0.05, 0.05, 0.3, 8]} /> <meshStandardMaterial emissive="yellow" emissiveIntensity={2} color="gold" /> </mesh> </Trail> </mesh> );
}
...
const RealisticRocket = () => {
  ...
  return (
    <group name="realistic-rocket">
      {/* Nozzle: base at z=0, extends to z=NOZZLE_BELL_HEIGHT */}
      ...
      {/* Lower Stage: base at z=NOZZLE_BELL_HEIGHT */}
      <mesh position={[0, 0, lowerStageCenterZ]}  material={materials.bodyWhite} castShadow receiveShadow > 
      ...
      {/* Upper Stage */}
      <mesh position={[0, 0, upperStageCenterZ]} material={materials.bodyGray} castShadow receiveShadow > 
      ...
      <mesh position={[0, 0, payloadFairingCenterZ]} material={materials.bodyWhite} castShadow receiveShadow > 
         {/* Cone's origin is its base, so position it at payloadFairingBaseZ + height/2 for correct stacking */}
        <coneGeometry args={[PAYLOAD_FAIRING_RADIUS_BASE, PAYLOAD_FAIRING_HEIGHT, 32]} /> 
      </mesh>
      
      {/* Fins (4x) - positioned radially around the lower stage */}
      {/* Fin 1 (+X) */}
      <mesh position={[LOWER_STAGE_RADIUS + FIN_SPAN / 2 - FIN_THICKNESS / 2, 0, finCenterZ]} material={materials.finMaterial} castShadow receiveShadow> 
        <boxGeometry args={[FIN_SPAN, FIN_THICKNESS, FIN_CHORD_ROOT]} /> 
      </mesh>
      {/* Fin 2 (-X) */}
      <mesh position={[-(LOWER_STAGE_RADIUS + FIN_SPAN / 2 - FIN_THICKNESS / 2), 0, finCenterZ]} material={materials.finMaterial} castShadow receiveShadow> 
        <boxGeometry args={[FIN_SPAN, FIN_THICKNESS, FIN_CHORD_ROOT]} /> 
      ...
      {/* Fin 3 (+Y) */}
      <mesh position={[0, LOWER_STAGE_RADIUS + FIN_SPAN / 2 - FIN_THICKNESS / 2, finCenterZ]} material={materials.finMaterial} castShadow receiveShadow> 
        <boxGeometry args={[FIN_THICKNESS, FIN_SPAN, FIN_CHORD_ROOT]} /> 
      </mesh>
      {/* Fin 4 (-Y) */}
      <mesh position={[0, -(LOWER_STAGE_RADIUS + FIN_SPAN / 2 - FIN_THICKNESS / 2), finCenterZ]} material={materials.finMaterial} castShadow receiveShadow> 
        <boxGeometry args={[FIN_THICKNESS, FIN_SPAN, FIN_CHORD_ROOT]} /> 
...
const ATTACK_PRELAUNCH_DUR = 2; const ATTACK_VERTICAL_ASCENT_DUR = 7; const ATTACK_APPROACH_TARGET_DUR = 10; const ATTACK_FIRING_DURATION = 5; const ATTACK_MISSION_COMPLETE_DUR = 5;
...
const RocketVehicle = ({ targetShipRef, onFireMissile, onMissionComplete }: RocketVehicleProps) => {
  ...
  const phaseStartTime = useRef(0); const lastMissileFireTime = useRef(0); const missileInterval = 0.4;
        ...
        if (isTargetCurrentlyValid) { 
          ...
          if (phaseForThisFrameLogic === 'firing-missiles') {
            if (currentTime - lastMissileFireTime.current > missileInterval) {
                ...
                const missileLocalStartOffset = new THREE.Vector3(0, 0, ROCKET_TOTAL_LENGTH ); 
                const missileStart = vehicleRef.current.localToWorld(missileLocalStartOffset.clone());
                const randomOffset = new THREE.Vector3(Math.random()-0.5, Math.random()-0.5, Math.random()-0.5).normalize().multiplyScalar(UFO_MAX_RADIUS * 0.5);
                const missileTarget = tgtPos.add(randomOffset); 
...
const SAT_PRELAUNCH_DUR = 1.5; 
const SAT_VERTICAL_ASCENT_DUR = 6; 
const SAT_GRAVITY_TURN_DUR = 8; 
const SAT_DEPLOY_DUR = 2; 
const SAT_DEORBIT_BURN_DUR = 2; 
const SAT_RETURN_FLIGHT_DUR = 10; 
const SAT_MISSION_COMPLETE_DELAY = 1; 
...
function Environment() {
  ...
  const handleFireMissile = useCallback((startPos: THREE.Vector3, targetPos: THREE.Vector3) => { 
    if (!alienShipRef.current || alienShipRef.current.status !== "operational") { 
        // console.warn("Attempted to fire missile, but alien ship is not operational or ref is null.");
        // Allow firing even if target is quickly destroyed, missile will just miss or hit debris
    }
    const id = `m-${performance.now()}`; 
    setMissiles(prev => [...prev, { key: id, id, startPosition: startPos, targetPosition: targetPos, onHit: handleMissileHit, onMiss: (mId) => setMissiles(ms => ms.filter(m => m.id !== mId)) }]); 
  }, []); 
